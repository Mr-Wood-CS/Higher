...
The **control unit (CU)** is responsible for directing the processor's work. It does not usually perform calculations itself. Its job is to coordinate the other parts of the processor and send control signals.
...
The **arithmetic and logic unit (ALU)** carries out arithmetic and logical operations.
...
When a program is running, its instructions and current data are held in main memory. The processor can then fetch instructions and data from memory as needed.
...
**RAM** is the computer's working memory. It stores the programs and data currently being used.
...
**ROM** stores instructions or data that should not be lost when the computer is switched off. It is **non-volatile**.
...
!!! example "Opening a saved file"
    ...
    When you open a saved document, it is read from secondary storage and copied into RAM. The processor then works with the copy in RAM while the document is open. When you save the document, the updated version is written back to secondary storage.
...
!!! example "Reading from memory"
    ...
    To read an instruction from memory, the processor places the memory address on the **address bus**. It sends a read signal on the **control bus**. The instruction stored at that address travels back to the processor on the **data bus**.

The three buses work together. The address bus alone is not enough because it only identifies a location. The data bus alone is not enough because the system also needs to know where the data is coming from or going to. The control bus tells the other components what kind of action should happen.
...
!!! example "Why cache helps"

    If a program repeats the same calculation many times, the same instructions may be needed again and again. Cache can keep copies of those instructions close to the processor, so they can be fetched more quickly.
