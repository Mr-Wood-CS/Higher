...
for index in range(len(member_names)):
    print(f"{member_names[index]}: {sessions_attended[index]} sessions")
...
for index in range(len(pupil_names)):
    print(f"{index}: {pupil_names[index]} - {distances_km[index]} km")
...
def display_equipment(equipment_names, total_owned, on_loan):
    for index in range(len(equipment_names)):
        ...
        if available == 0:
            print(f"{equipment_names[index]}: {available} available - None available")
        else:
            print(f"{equipment_names[index]}: {available} available")
...
while correction_index < 0 or correction_index >= len(pupil_names):
    print(f"Enter an index from 0 to {len(pupil_names) - 1}.")
    correction_index = int(input("Enter the pupil index to correct: "))
