...
??? info "Append Mode"
	...
	:   __"a" (Append Mode)__ is used to add new data to the end of the file.

	:	Use `open()` with mode `"a"` to open the file in append mode.
	...
	Append mode is useful, but it is additional to the named Higher operations:
